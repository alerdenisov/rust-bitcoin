...
impl fmt::Display for ExtendedPrivKey {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        let mut ret = [0; 78];
        ret[0..4].copy_from_slice(&self.network.xprv_magic().ok_or(fmt::Error)?[..]);
...
impl FromStr for ExtendedPrivKey {
    ...
    fn from_str(inp: &str) -> Result<ExtendedPrivKey, base58::Error> {
        ...
        if let Some(network) = Network::from_xprv(&data[0..4]) {
            Ok(ExtendedPrivKey {
                network: network,
                depth: data[4],
            ...
            Err(base58::Error::InvalidVersion((&data[0..4]).to_vec()))
        }
    }
}
...
impl fmt::Display for ExtendedPubKey {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        let mut ret = [0; 78];
        ret[0..4].copy_from_slice(&self.network.xpub_magic().ok_or(fmt::Error)?[..]);
        ret[4] = self.depth as u8;
        ret[5..9].copy_from_slice(&self.parent_fingerprint[..]);
        ret[9..13].copy_from_slice(&endian::u32_to_array_be(u32::from(self.child_number)));
        ret[13..45].copy_from_slice(&self.chain_code[..]);
...
impl FromStr for ExtendedPubKey {
    ...
    fn from_str(inp: &str) -> Result<ExtendedPubKey, base58::Error> {
        ...
        if let Some(network) = Network::from_xpub(&data[0..4]) {
            Ok(ExtendedPubKey {
                network: network,
                depth: data[4],
                parent_fingerprint: Fingerprint::from(&data[5..9]),
